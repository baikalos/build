...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^baikalos_") ; then
        ...
        export BUILD_NUMBER=$( (date +%s%N ; echo $BAIKALOS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        BAIKALOS_BUILD=
    fi
...
function setpaths()
{
    ...
    ANDROID_BUILD_PATHS=$ANDROID_BUILD_PATHS:$ANDROID_KERNEL_TOOLCHAIN_PATH:
...
function print_lunch_menu()
{
    ...
    echo "|  _ \        (_)| |         | | / __ \  / ____|"
    echo "| |_) |  __ _  _ | | __ __ _ | || |  | || (___  "
    echo "|  _ <  / _` || || |/ // _` || || |  | | \___ \ "
    echo "| |_) || (_| || ||   <| (_| || || |__| | ____) |"
    echo "|____/  \__,_||_||_|\_\\__,_||_| \____/ |_____/ "
    echo "================================================"
    tput sgr0;
    echo ""
    ...
    if [ "z${BAIKALOS_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    BAIKALOS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/baikalos/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
